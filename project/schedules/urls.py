...
urlpatterns = patterns('project.schedules.views',
    ...
    url(r'^week/(?P<channel>\w+)/(?P<year>\d{4})/(?P<month>\d{1,2})/(?P<day>\d+)/$', 'channel_week',
        name="channel-week-glance"),
    url(r'^(?P<channel>\w+)/(?P<year>\d{4})/(?P<month>\d{1,2})/(?P<day>\d+)/$', 'channel', name="channel-date"),
    url(r'^(?P<channel>\w+)/$', 'channel', name="channel-today"),
    url(r'^(?P<year>\d{4})/(?P<month>\d{1,2})/(?P<day>\d+)/$', 'schedules_main', name="schedules-date"),
