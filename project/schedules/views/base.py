...
class Index(SchedulesPage):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        primetime = [
            {"service": service, "day": day, "airing": service.primetime_for_day(day)}
...
class ChannelView(ChannelMixin, SchedulesPage):
    ...
    def get_page_title(self):
        return "%s | %s" % (self.channel.name, self.base_title)
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        airing = (
            EpisodeAir.objects.select_related(
                "service", "episode", "episode__season", "episode__season__series"
            )
            .filter(service=self.channel)
            .filter(date=day)
            .order_by("date", "time")
        )
        ...
        last_time = EpisodeAir.objects.filter(service=self.channel).aggregate(
            latest=Max("date")
        )
...
class ChannelWeek(ChannelMixin, SchedulesPage):
    ...
    def get_page_title(self):
        return "Weekly Schedule for %s | %s" % (self.channel.name, self.base_title)
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        airing = (
            EpisodeAir.objects.select_related()
            .filter(service=self.channel)
            .filter(date__range=[week[0], week[len(week) - 1]])
            .order_by("date", "time")
            .values(
                "date",
                "time",
                "episode__id",
                "episode__short_description",
                "episode__season__series__name",
            )
        )
...
class EpisodeDetail(SchedulesPage):
    ...
    def get_page_title(self):
        return " Episode of %s | %s" % (
            self.episode.season.series.name.strip(),
            self.base_title,
        )
    ...
    def get(self, request, *args, **kwargs):
        ...
        if "episode" in kwargs:
            ...
            self.episode = get_object_or_404(
                Episode.objects.select_related("season", "season__series"), pk=id
            )
        else:
            raise Http404
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        future = (
            self.episode.airs.filter(date__gte=today)
            .values("service__name", "service__keyname", "time", "date")
            .order_by("date", "time")
        )
        past = (
            self.episode.airs.filter(date__lt=today)
            .values("service__name", "service__keyname", "time", "date")
            .order_by("date", "time")[:25]
        )
...
class SeriesAirs(SchedulesPage):
    ...
    def get_page_title(self):
        return "%s | %s" % (self.series[0].name, self.base_title)
        ...
        for i in range(0, len(self.series)):
            try:
                airs = (
                    ...
                    .filter(
                        episode__season__series=self.series[i],
                        date__range=[today, date_future],
                    )
                    .order_by("date", "time")
                )
            except:
...
class Search(SchedulesPage, FormView):
    ...
    def get_page_title(self):
        return "Search | %s" % self.base_title
    ...
    def get_context_data(self, **kwargs):
        ...
        c = {"form": form, "query": query, "results": results}
    ...
    def form_valid(self, form):
        ...
        terms = [
            ("" if t[0] not in "+-" else t[0]) + t.strip(punctuation)
        ...
        ep_search_fields = {"name": 5, "short_description": 1}

        def build_filters(fields):
            # Create the queryset combining each set of terms.
            excluded = [
                reduce(iand, [~Q(**{"%s__icontains" % f: t[1:]}) for f in fields])
                for t in terms
                if t[0] == "-"
            ]
            required = [
                reduce(ior, [Q(**{"%s__icontains" % f: t[1:]}) for f in fields])
                for t in terms
                if t[0] == "+"
            ]
            optional = [
                reduce(ior, [Q(**{"%s__icontains" % f: t}) for f in fields])
                for t in terms
                if t[0] not in "+-"
            ]
        ...
        if service:
            queryset = Series.objects.filter(
                episode__airs__date__range=date_range,
                episode__airs__service__typing="tv",
                episode__airs__service=service,
            )
        else:
            queryset = Series.objects.filter(
                episode__airs__date__range=date_range,
        ...
        series_excluded, series_required, series_optional = build_filters(
            series_search_fields
        )
        if series_excluded:
        ...
        for series in queryset:
            if service:
                check = EpisodeAir.objects.filter(
                    episode__series=series, date__range=date_range, service=service
                )
            else:
                check = EpisodeAir.objects.filter(
                    episode__series=series, date__range=date_range
                )
            ...
            if service:
                episodes = Episode.objects.filter(
                    airs__date__range=date_range, series=series, airs__service=service
                ).distinct()
            else:
                episodes = Episode.objects.filter(
                    airs__date__range=date_range, series=series
                ).distinct()
        ...
        results = sorted(
            results, key=lambda k: k["series"].score, reverse=True
        )  # actually orders for results, reverse because big #s == higher score
    ...
    def form_invalid(self, form):
        ...
        c = {"form": form, "query": query, "results": None}
...
class PrintCenter(SchedulesPage, FormView):
    ...
    def get_page_title(self):
        return "Print | %s" % self.base_title
    ...
    def form_valid(self, form):
        ...
        if form.cleaned_data["primetime_toggle"]:
            airs = airs.filter(
                time__range=[service.primetime_start, service.primetime_end]
            )
...
class API(TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        self.tpl = request.GET.get(
            "tpl", "motnbase"
        )  # use default mothnbase if no other template specified
        self.svc = request.GET.get("svc", None)

        if self.action == "billboard":
            try:
                self.the_date = datetime.strptime(get_datetime, "%Y-%m-%d_%H.%M.%S")
            except (ValueError, TypeError):
                raise Http404("datetime in wrong format (dt=%Y-%m-%d_%H.%M.%S)")

            if not self.the_date:
                raise Http404(
                    "API call wrong-- usage: /schedules/api/?action=billboard&dt=2010-05-01_21.00.00&tpl=base.htm"
                )
        ...
        if not self.action:
            raise Http404(
                "API call wrong no action requested -- usage : /schedules/api/?action=ACTION"
            )
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.action == "billboard":
            ...
            for service in services:
                ...
                today_airs = airs.filter(
                    date__range=(self.the_date, self.the_date + timedelta(days=1))
                )
                tomorrow_airs = airs.filter(
                    date__range=(
                        self.the_date + timedelta(days=1),
                        self.the_date + timedelta(days=2),
                    )
                )

                # queries to get what's "on now"
                try:
                    airing_now = airs.filter(
                        time__lte=self.the_time, date=self.the_date
                    )
                    airing_tomorrow = airs.filter(
                        time__lte=self.the_time, date=self.the_date + timedelta(days=1)
                    )
                except:
                ...
                if not airing_now:
                    airing_now = airs.filter(
                        date=self.the_date + timedelta(days=-1)
                    ).order_by("-time")
                ...
                try:
                    today_1 = today_airs.filter(
                        date=self.the_date, time__gte=airing_now.latest("time").time
                    ).order_by("date", "time")
                    today_2 = today_airs.filter(
                        date=self.the_date + timedelta(days=1)
                    ).order_by("date", "time")
                        ...
                        this_service.append(
                            {
                                "entry": {
                                    "episode": {
                                        "season": {
                                            "series": {
                                                "name": today[0].airing.series.name
                                            }
                                        }
                                    }
                                },
                                "title_object": [
                                    ord(today[0].airing.series.name[i].decode("U8"))
                                    for i in range(0, len(today[0].airing.series.name))
                                ],
                                "time_object": [
                                    ord(
                                        today[0]
                                    ...
                                    for i in range(
                                        0,
                                        len(
                                            today[0].time.strftime("%l:%M %p").lstrip()
                                        ),
                                    )
                                ],
                                "time_string": today[0]
                        ...
                        this_service.append(
                            {
                                "entry": {
                                    "episode": {
                                        "season": {"series": {"name": "See website"}}
                                    }
                                },
                                "title_object": [
                                    ord("See website"[i].decode("U8"))
                                    for i in range(0, len("See website"))
                                ],
                                "time_object": [
                                    ord(
                                        self.the_time.strftime("%l:%M %p")
                                        .lstrip()[i]
                                        .decode("U8")
                                    )
                                    for i in range(
                                        0,
                                        len(
                                            self.the_time.strftime("%l:%M %p").lstrip()
                                        ),
                                    )
                                ],
                                "time_string": self.the_time.strftime(
                                    "%l:%M %p"
                                ).lstrip(),
                            }
                        )
                        ...
                        this_service.append(
                            {
                                "entry": {
                                    "episode": {
                                        "season": {
                                            "series": {
                                                "name": today[1].airing.series.name
                                            }
                                        }
                                    }
                                },
                                "title_object": [
                                    ord(today[1].airing.series.name[i].decode("U8"))
                                    for i in range(0, len(today[1].airing.series.name))
                                ],
                                "time_object": [
                                    ord(
                                        today[1]
                                    ...
                                    for i in range(
                                        0,
                                        len(
                                            today[1].time.strftime("%l:%M %p").lstrip()
                                        ),
                                    )
                                ],
                                "time_string": today[1]
                        ...
                        this_service.append(
                            {
                                "entry": {
                                    "episode": {
                                        "season": {
                                            "series": {
                                                "name": today[2].airing.series.name
                                            }
                                        }
                                    }
                                },
                                "title_object": [
                                    ord(today[2].airing.series.name[i].decode("U8"))
                                    for i in range(0, len(today[2].airing.series.name))
                                ],
                                "time_object": [
                                    ord(
                                        today[2]
                                    ...
                                    for i in range(
                                        0,
                                        len(
                                            today[2].time.strftime("%l:%M %p").lstrip()
                                        ),
                                    )
                                ],
                                "time_string": today[2]
                        ...
                        this_service.append(
                            {
                                "entry": {
                                    "episode": {
                                        "season": {
                                            "series": {
                                                "name": today[3].airing.series.name
                                            }
                                        }
                                    }
                                },
                                "title_object": [
                                    ord(today[3].airing.series.name[i].decode("U8"))
                                    for i in range(0, len(today[3].airing.series.name))
                                ],
                                "time_object": [
                                    ord(
                                        today[3]
                                    ...
                                    for i in range(
                                        0,
                                        len(
                                            today[3].time.strftime("%l:%M %p").lstrip()
                                        ),
                                    )
                                ],
                                "time_string": today[3]
                    ...
                    this_service.append(
                        {
                            ...
                            "time_object": [
                                ...
                                for i in range(
                                    0, len(self.the_time.strftime("%l:%M %p").lstrip())
                                )
                            ],
                            "time_string": self.the_time.strftime("%l:%M %p").lstrip(),
                        }
                    )
                    this_service.append({"filler": "filler"})
                ...
                try:
                    tomorrow_1 = tomorrow_airs.filter(
                        date=self.the_date + timedelta(days=1),
                        time__gte=airing_tomorrow.latest("time").time,
                    ).order_by("date", "time")
                    tomorrow_2 = tomorrow_airs.filter(
                        date=self.the_date + timedelta(days=2)
                    ).order_by("date", "time")
                        ...
                        this_service.append(
                            {
                                "entry": {
                                    "episode": {
                                        "season": {
                                            "series": {
                                                "name": tomorrow[0].airing.series.name
                                            }
                                        }
                                    }
                                },
                                "title_object": [
                                    ord(tomorrow[0].airing.series.name[i].decode("U8"))
                                    for i in range(
                                        0, len(tomorrow[0].airing.series.name)
                                    )
                                ],
                                "time_object": [
                                    ord(
                        ...
                        this_service.append(
                            {
                                "entry": {
                                    "episode": {
                                        "season": {
                                            "series": {
                                                "name": tomorrow[1].airing.series.name
                                            }
                                        }
                                    }
                                },
                                "title_object": [
                                    ord(tomorrow[1].airing.series.name[i].decode("U8"))
                                    for i in range(
                                        0, len(tomorrow[1].airing.series.name)
                                    )
                                ],
                                "time_object": [
                                    ord(
        ...
        if self.action == "schedulexml":
            ...
            services = get_list_or_404(
                Service, active=True, keyname__in=desired_channels
            )

            airs = Air.objects.filter(
                service=services[0], date__range=date_range
            ).distinct()
